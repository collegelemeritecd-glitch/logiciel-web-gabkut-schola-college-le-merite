...
exports.getTeachers = async (req, res) => {
  console.log('🔵 [adminTeacherController] GET /api/admin/teachers');
  try {
    const teachers = await Teacher.find({}).sort({ fullName: 1 });
    res.json(teachers.map(serializeTeacher));
  } catch (err) {
    console.error('❌ [adminTeacherController] Erreur getTeachers admin:', err);
...
exports.getTeacherById = async (req, res) => {
  console.log(
    '🔵 [adminTeacherController] GET /api/admin/teachers/:id',
    'id =',
    req.params.id
  );
  try {
    ...
    console.error('❌ [adminTeacherController] Erreur getTeacherById admin:', err);
    res.status(500).json({ message: 'Erreur serveur' });
  }
};
...
exports.createTeacher = async (req, res) => {
  console.log('🔵 [adminTeacherController] POST /api/admin/teachers');
  ...
  try {
    ...
    if (!fullName || !subject) {
      console.warn(
        '⚠️ [adminTeacherController] fullName ou subject manquant côté body'
      );
      return res
        .status(400)
    ...
    console.log('🟢 [adminTeacherController] Document avant save =', {
    ...
    console.log(
      '✅ [adminTeacherController] Enseignant créé avec succès, id =',
    ...
    console.error('❌ [adminTeacherController] Validation enseignant:', err);
...
exports.updateTeacher = async (req, res) => {
  console.log(
    '🔵 [adminTeacherController] PUT /api/admin/teachers/:id',
    'id =',
    req.params.id
  );
  ...
  try {
    ...
    console.log(
      '✅ [adminTeacherController] Enseignant mis à jour, id =',
      teacher._id.toString()
    );
    ...
    console.error('❌ [adminTeacherController] Erreur updateTeacher admin:', err);
    res.status(500).json({ message: 'Erreur serveur' });
  }
};
...
exports.deleteTeacher = async (req, res) => {
  console.log(
    '🔵 [adminTeacherController] DELETE /api/admin/teachers/:id',
    'id =',
    req.params.id
  );
  try {
    ...
    console.log(
      '✅ [adminTeacherController] Enseignant supprimé, id =',
      teacher._id.toString()
    );

    res.json({ message: 'Enseignant supprimé avec succès.' });
  } catch (err) {
    console.error('❌ [adminTeacherController] Erreur deleteTeacher admin:', err);
    res.status(500).json({ message: 'Erreur serveur' });
  }
};
